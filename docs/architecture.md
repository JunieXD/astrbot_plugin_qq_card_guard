...
| `scheduling.py` | 缓存有效性与补查阶梯的纯决策，不授权写操作 |
...
4. 接入共享队列时，排队期间释放本地账号锁，排队结束后才重新确认账号并读取机器人权限、全员禁言、目标资料和入群身份。新处罚使用两次一致的群名片/入群时间核验。
...
禁言归属需要成功提交记录、相同入群身份、相符的截止时间，以及从提交前基线起唯一一条匹配的自身禁言通知。两次读取的禁言截止时间必须一致；同账号第二次操作也会使原归属失效。中途解除、延长或其他不匹配通知使归属失效。不因管理员提前解禁就在本轮补禁；下一轮依然需要新的发言和原有冷却结束。
...
- 普通启动只对新处罚缓冲；连接变化、网络异常和管理员恢复使用独立持久化冷却。小幅调整配置不会缩短已安排的截止时间。自定义等待与过期窗口分开计算，长延迟不会被当作处理过期。
...
事件去重表只保留48小时缓存，审计表不做时间清理。通知提示与当前接口资料冲突时先暂缓；较早的角色提示可以通过两次当前资料核验恢复。

## 缓存、补查与诊断

`subjects.screening` 保存实际核验时间、判定、群策略摘要、连接会话、入群代次及临时豁免截止。缓存命中只跳过无待办成员的新发言检查，不用于写操作授权。状态变更事件清理缓存并写审计；同一秒的新事件以持久化版本保护，旧任务完成不能清除新任务。

宽限内无名片变化的新发言合并入已有事项。同一成员的一次调度使用 ContextVar 隔离短时读取结果（最多10秒，版本/会话/规则一致才可复用），用于合并到期补查与发言检查；写入前的两次稳定核验始终绕过。无需跨成员或跨群共享成员资料。

补查以 `sent_at` 而非最新发言时间选阶梯。仍有本插件禁言时优先读取；名片变化可提前唤醒，已知读限流截止仍保留。准备写入时先获取可重入的适配器读锁，随后开始20秒时效计时，整个准备与提交期间不夹入其他成员的读取；取消会释放锁，通知接收不依赖该锁。

读取预算按最近60分钟总使用量与优先级门槛比较，恢复点取满足门槛所需淘汰的那一笔记录。提高/降低预算后重新调度限流任务并再次核验。状态指标仅存固定类别计数与最后一次详情，避免无限指标键增长。日志的严重程度字段保留，群等级使用 `member_level`。
